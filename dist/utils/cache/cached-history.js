...
export class HistoryFetcher {
    ...
    async getHistory(activeWallet, mneeInstance, options = {}) {
        ...
        // Fetch data from API
        const { history } = await mneeInstance.recentTxHistory(activeWallet.address, undefined, limit * 2, 'desc');
    ...
    applyLimit(transactions, limit) {
        if (limit > 0) {
            return transactions.slice(0, limit);
        }
        return transactions;
    }
    async filterHistory(activeWallet, mneeInstance, filters = {}) {
        // Get full history
        ...
        let filtered = history.filter((tx) => {
            if (filters.confirmed && tx.status !== 'confirmed')
                return false;
            if (filters.unconfirmed && tx.status !== 'unconfirmed')
                return false;
            ...
            if (filters.address) {
                const hasAddress = tx.computedCounterparties.some((cp) => cp.address.toLowerCase().includes(filters.address.toLowerCase()));
                if (!hasAddress)
                    return false;
            }
            if (filters.min !== undefined && tx.computedAmount < filters.min)
                return false;
            if (filters.max !== undefined && tx.computedAmount > filters.max)
                return false;
            return true;
        });
    ...
    async clearCache(walletAddress, environment) {
        await this.cache.clear(walletAddress, environment);
    }
    async getCacheInfo(walletAddress, environment) {
