...
program
  ...
  .action(async (options) => {
    ...
    if (!activeWallet) {
      console.error(
        `${icons.error} ${colors.error('No active wallet found.')} Run ${colors.primary('mnee create')} first.`
      );
      return;
    }
    ...
    const spinner = createSpinner(
      options.refresh 
        ...
        : `Fetching history for ${colors.primary(activeWallet.name)}...`
    );
    spinner.start();
    ...
    try {
      ...
      history.forEach((tx, index) => {
        ...
        if (tx.height) console.log(`     ${colors.muted(`block ${tx.height}`)}`);
