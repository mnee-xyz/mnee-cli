...
import { SendMNEE } from "mnee-ops/dist/mnee.types.js";
...
program
  ...
  .action(async () => {
    try {
      const existingAddress = await keytar.getPassword(
        SERVICE_NAME,
        "walletAddress"
      );
      if (existingAddress) {
        console.error(
          "❌ Wallet already exists. Run `mnee export-key` to retrieve keys."
        );
        return;
      }
...
program
  ...
  .action(async () => {
    ...
    const { decimalAmount } = await mneeClient.balance(address);
...
program
  ...
  .action(async () => {
    try {
      ...
      const { txid, error } = await mneeClient.transfer(
        request,
        privateKey.toWif()
      );
