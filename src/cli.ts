...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      let history: any[] = [];
      let nextScore: number | undefined = undefined;
      ...
      const filterTx = (tx: any): boolean => {
        if (options.unconfirmed && tx.status !== 'unconfirmed') return false;
        if (options.confirmed && tx.status !== 'confirmed') return false;
        if (searchType && tx.type !== searchType) return false;
        if (searchTxId && !tx.txid.toLowerCase().includes(searchTxId)) return false;
        if (searchAddress && !tx.computedCounterparties.some((cp: any) => cp.address.toLowerCase().includes(searchAddress))) return false;
        if (options.min !== undefined && tx.computedAmount < options.min) return false;
        if (options.max !== undefined && tx.computedAmount > options.max) return false;
        return true;
      };

      const computeAmount = (tx: TxHistory, mneeInstance: any) => ({
        ...tx,
      ...
      while (!foundEnough && apiCalls < maxCalls) {
        ...
        if (newHistory.length === 0 || newNextScore === nextScore) break;

        const enrichedNewHistory = newHistory.map((tx: TxHistory) => computeAmount(tx, mneeInstance));
        ...
        if (nextScore === 0 || nextScore === undefined) break;
      ...
      history.forEach(tx => {
        ...
        if (!existing || tx.score > existing.score) txMap.set(tx.txid, tx);
      });
      ...
      history.forEach((tx, index) => {
        ...
        const amount = tx.computedAmount;
        ...
        if (fee > 0) console.log(`     ${colors.muted('fee:')} ${formatAmount(fee)}`);

        tx.computedCounterparties.forEach((cp: any) => {
          console.log(`     ${colors.muted(type === 'send' ? 'to:' : 'from:')} ${colors.muted(cp.address)} ${formatAmount(cp.computedAmount)}`);
        });

        if (heightDisplay) console.log(`     ${colors.muted(heightDisplay)}`);
        console.log(`     ${colors.muted(`tx: ${tx.txid}`)}`);

        if (index < history.length - 1) console.log(colors.muted('     ' + '·'.repeat(50)));
        console.log('');
      });
